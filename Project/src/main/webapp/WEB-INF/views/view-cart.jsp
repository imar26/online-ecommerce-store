...
<script>
	$(document).ready(function() {
		$(".delete").on('click', function(e) {
			...
			var prd_qty = $(this).parent().parent().find('.prd_quantity').text();
			...
			$.ajax({
		        ...
		        data: { 'productID': productID, 'prd_qty' : prd_qty },
		        type: 'GET',
		        cache: false,
		        success: function (response) {
		        	$("#row-"+response).fadeOut();
		        	totalPrice = (Math.abs(totalPrice).toFixed(2)) - (Math.abs(qty).toFixed(2));
		...
		<c:when test="${!empty products}">
			<table id="table" border="1" cellpadding="5">
				...
				<c:forEach var="product" items="${products}">
					<tr id="row-${product.product.productID}" data-id="${product.product.productID}">
						<td>${product.product.productName}</td>
						<td>${product.product.productPrice}</td>
						<td class="prd_quantity">${product.quantity}</td>
						<td class="quantity"><c:set var="totalPrice" value="${product.product.productPrice * product.quantity}" />${product.product.productPrice * product.quantity}</td>
